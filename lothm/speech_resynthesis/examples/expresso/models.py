...
from speech_resynthesis.examples.speech_to_speech_translation.models import (
...
class MultiSpkrMultiStyleCodeGenerator(DurationCodeGenerator):
    ...
    def __init__(self, h):
        ...
        if self.multispkr:
            self.spkr = nn.Linear(h.speaker_vector_dim, h.embedding_dim)
        if self.multistyle:
    ...
    def forward(self, **kwargs):
        ...
        if self.multispkr:
            spkr = self.spkr(kwargs["spkr"]).unsqueeze(-1)
...
class MultiSpkrMultiAccentCodeGenerator(DurationCodeGenerator):
    """
    Discrete unit-based HiFi-GAN vocoder with one-hot expression conditioning.
    (used in the Expresso paper)
    The current implementation only supports unit, speaker ID and
    ...
    def __init__(self, h):
        ...
        if self.multistyle:
            self.style = nn.Embedding(h.get("num_styles", 100), h.embedding_dim)
        self.accent = nn.Embedding(2, h.embedding_dim)
    ...
    def forward(self, **kwargs):
        ...
        if self.multistyle:
            ...
            style = self._upsample(style, x.shape[-1])
            x = torch.cat([x, style], dim=1)
        accent = self.accent(kwargs["accent"]).transpose(1, 2)
