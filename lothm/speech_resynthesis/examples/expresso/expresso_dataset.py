...
from speech_resynthesis.dataset import parse_speaker, load_audio, mel_spectrogram, MAX_WAV_VALUE
...
class MultiSpkrMultiStyleCodeDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.audio_files, self.codes, self.vectors = training_files
        ...
        if self.multispkr:
            if self.multispkr == "xvector":
                self.xvector_classifier = xvec_cls
            
            elif self.multispkr != "from_input_file":
                self.spkr_names = [
    ...
    def __getitem__(self, index):
        ...
        speaker_vector = self.vectors[index]

        assert audio.size(1) >= self.segment_size, "Padding not supported!!"
        # print(f"xxx audio:{audio} code: {code}")
        ...
        if self.multispkr:
            feats["spkr"] = speaker_vector

        if self.multistyle:
            feats["style"] = self._get_style(index)

        if "vctk" in str(filename) or "LJ" in str(filename):
            feats["accent"] = torch.LongTensor([0]).view(1).numpy() #"eng"
        else:
            feats["accent"] = torch.LongTensor([1]).view(1).numpy() #"heb"
...
class InferenceCodeDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.xvector_classifier = SpeakerRecognition.from_hparams(source="speechbrain/spkrec-ecapa-voxceleb")
    ...
    def __getitem__(self, index):
        ...
        if self.multispkr:
            if not self.multispkr == "xvector":
                ...
                feats["spkr"] = np.array(self.xvector_classifier.encode_batch(torch.tensor(audio)).squeeze())
        ...
        if self.accent:
            feats["accent"] = torch.LongTensor([map_lang[self.accent]]).view(1).numpy()
