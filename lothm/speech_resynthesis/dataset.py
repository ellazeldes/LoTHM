...
def parse_manifest(manifest):
        ...
        for line in info.readlines():
            if line[0] == '{':
                ...
                if sample[k]:
                    ...
                    if "speaker_vector" in sample:
                        xvectors += [torch.Tensor(
                            sample["speaker_vector"]
                        ).numpy()]
                else:
                    codes = np.array([])
    ...
    return audio_files, codes, xvectors

def get_audio_names(h):
    return get_dataset_filelist(h, False)

def get_dataset_filelist(h, codes=True):
    training_files, training_codes, training_vectors = parse_manifest(h.input_training_file)
    validation_files, validation_codes, validation_vectors = parse_manifest(h.input_validation_file)
    if not codes:
        training_codes = []
        validation_codes = []
    return (training_files, training_codes, training_vectors), (validation_files, validation_codes, validation_vectors)
...
def parse_speaker(path, method):
    ...
    if method == 'parent_name':
        ...
        return 'A'
    elif method == "heb_data":
        if 'fleurs' in path.parts:
            return 'fleurs' 
        else: 
            return path.parts[7]
...
class CodeDataset(torch.utils.data.Dataset):
    def __init__(self, training_files, segment_size, code_hop_size, n_fft, num_mels,
                 ...
                 f0_interp=False, vqvae=False, on_the_fly=True):
        ...
        if self.on_the_fly:
            # self.dense_model_name = "hubert-base-ls960-layer-9"
            self.dense_model_name = "mhubert-base-vp_mls_cv_8lang"
            
            self.km_checkpoint_path = None
            # self.dense_model_name = "hubert-base-heb-layer-9"
            # self.km_checkpoint_path = "/cs/labs/adiyoss/ellaerez/stlothm/shaul_speech_resynthesis/sampled_data_9l_500cl_base_old_hubert.km"
            self.quantizer_name, self.vocab_size = "kmeans", 2000 #500
            self.encoder = SpeechEncoder.by_name(
                dense_model_name=self.dense_model_name,
                quantizer_model_name=self.quantizer_name,
