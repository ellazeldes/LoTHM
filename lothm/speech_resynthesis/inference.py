...
from speech_resynthesis.dataset import CodeDataset, parse_manifest, mel_spectrogram, \
    MAX_WAV_VALUE
from speech_resynthesis.utils import AttrDict
from speech_resynthesis.models import CodeGenerator
...
def init_worker(queue, arguments):
    ...
    if a.code_file is not None:
        ...
        dataset = [(parse_code(x[1].strip()), None, x[0], None) for x in dataset]
    else:
        ...
        dataset = CodeDataset(file_list, -1, h.code_hop_size, h.n_fft, h.num_mels, h.hop_size, h.win_size,
                              ...
                              pad=a.pad, on_the_fly=a.onthefly)
...
def main():
    ...
    parser.add_argument('--onthefly', type=bool)
