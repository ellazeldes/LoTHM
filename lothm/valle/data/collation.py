...
class AlephBertCollater:
    ...
    def __init__(
        ...
        self.alephbert_tokenizer = BertTokenizer.from_pretrained('onlplab/alephbert-base')
    ...
    def __call__(self, texts: List[str]) -> Tuple[torch.Tensor, torch.Tensor]:
        tokens_seqs = [self.alephbert_tokenizer(text, return_tensors="pt").input_ids[0] for text in texts]
        # print(f"collator - {tokens_seqs}")
        max_len = len(max(tokens_seqs, key=len))
        seqs = [
            list(seq)
            + [0] * (max_len - len(seq))
        ...
        # print(f"tokens_batch - {tokens_batch}")
        ...
        # print(f"collator end - {tokens_batch}")

        return tokens_batch, tokens_lens


def get_text_token_collater(text_tokens_file: str) -> TextTokenCollater:
    if text_tokens_file == "alephbert":
         return AlephBertCollater()
...
if __name__ == '__main__':
    collator = AlephBertCollater()
    print(collator(["מה קורה יגבר", "היי מה הולך"]))

    text_collator = TextTokenCollater("")