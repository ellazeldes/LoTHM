...
class AudioTokenizer:
    ...
    def encode(self, wav: torch.Tensor) -> torch.Tensor:
        ...
        # z, codes, latents, _, _ = self.codec_dac.encode(wav.to("cpu"))
        # print(f"debug - {res[0][0].shape} - {codes.shape} - {type(res[0][0])} - {type(codes)}")
        return res
...
class DacAudioTokenizer:
    ...
    def encode(self, wav: torch.Tensor) -> torch.Tensor:
        z, codes, latents, _, _ = self.codec.encode(wav.clone().to(self.device))
        # print(f"debug dac tokens {codes.shape}: {codes}")
        res = [(codes, None)]
        # print(f"debug dac - {res}")
        return res

    def decode(self, codes: torch.Tensor) -> torch.Tensor:
        print(codes[0][0])
        print(codes[0][0].shape)
        z = self.codec.quantizer.from_codes(codes[0][0])
        return self.codec.decode(z[0])
