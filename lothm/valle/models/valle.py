    ...
    def inference(
        ...
        while True:
            ...
            if top_k > 0:
                ...
                samples = repetition_aware_sampling(logits, preceding_sequence, window_size, top_p_value, repetition_threshold, temperature)
            ...
            y = torch.concat([y, samples], dim=1)
            preceding_sequence = torch.concat([preceding_sequence, samples], dim=1)
...
def repetition_aware_sampling(logits, preceding_sequence, window_size, top_p_value, repetition_threshold, temperature):
    """
    Perform repetition-aware sampling as described in the provided method.
    
    Args:
        probs (np.ndarray): The probability distribution over the vocabulary.
        preceding_sequence (list): The list of previously generated tokens.
        window_size (int): The window size for calculating the repetition ratio.
        top_p_value (float): The top-p value for nucleus sampling.
        repetition_threshold (float): The repetition threshold ratio.
    ...
    logits = top_k_top_p_filtering(logits, top_k=0, top_p=top_p_value, filter_value=-float("Inf"), min_tokens_to_keep=1)
    token = torch.multinomial(F.softmax(logits, dim=-1), num_samples=1)
    # Calculate the repetition ratio
    window_start = max(0, preceding_sequence.shape[1] - window_size)
    if preceding_sequence.shape[0] == 0:
        window = preceding_sequence.flatten()
    else:
        window = preceding_sequence.flatten()[window_start:]
    repetition_ratio = torch.bincount(window, minlength=2001)[token] / window_size
    
    # Check if the repetition ratio exceeds the threshold
    if repetition_ratio > repetition_threshold:
        # Replace the target code by random sampling from the original distribution
        token = torch.multinomial(F.softmax(orig_logits, dim=-1), num_samples=1)
    return token